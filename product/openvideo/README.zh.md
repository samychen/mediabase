...
- **上游同款界面**：项目卡片主页（封面=首片段首帧）+ 四区编辑器（左媒体栏/中播放器/
  右检查器/底通栏时间线），由产品自有壳 `@openvideo/ui-shell` 渲染；基座外壳为此中立地
  扩展了 `right`/`bottom` 两个面板区（任何产品可用，不注册则布局与从前逐像素一致）。
...
- **导出 MP4/WebM**：浏览器端 canvas + MediaRecorder 实时录制（本地替代上游的托管导出服务；
  草稿级保真，见「边界」）。
- **一键换肤**：基座 `@mediabase/theme`（暗色/午夜/浅色/工作室/工作室·浅色五套设计令牌皮肤，header 下拉即换、浏览器记忆；本产品默认"工作室"——上游 OpenVideo 的暖调暗色+玫红点缀+墨色按钮风格，由名册行配置决定）。
