...
- **Export to MP4/WebM**: browser-side canvas + MediaRecorder, real time (the
  ...
  Boundaries).
- **One-click skins**: the base's `@mediabase/theme` (dark/midnight/light/studio/studio-light design-token skins, header picker, remembered per browser; this product defaults to studio — the upstream OpenVideo look — via its roster row).
