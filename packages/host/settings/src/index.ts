...
// ~/.avstudio/settings.json by default (config.file overrides). Consumers read
...
export interface SettingsConfig {
  /** JSON file path; defaults to ~/.avstudio/settings.json. */
  file?: string
}
...
export const Config: Schema<SettingsConfig, SettingsConfig> = z.object({
  file: z.string().description('settings JSON path; default ~/.avstudio/settings.json'),
})

export function apply(ctx: Context, rawConfig: SettingsConfig = {}): void {
  const config = parse(Config, rawConfig)
  const file = config.file ?? join(homedir(), '.avstudio', 'settings.json')
  ...
  ctx.capabilities.register({
    id: 'settings',
    title: '持久化设置',
    description: '键值设置落盘 ~/.avstudio/settings.json(明文),供 AI 助手等能力读取',
    services: ['settings'],
